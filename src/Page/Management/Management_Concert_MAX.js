...
const Management_Concert_MAX = () => {
    ...
    const navigateToPPageMΙΝ = () => {
        navigate("/management_concert_min");
    };
    ...
    useEffect(() => {
        if (userId) {
            const fetchConcerts = async () => {
                try {
                    const response = await axios.get(`${API_URL}/concertsU-MAX`, {
                        params: {
                            id_user: userId,
                ...
                <div className="relative overflow-x-auto shadow-md" style={{ maxWidth: '1280px', margin: '20px auto', padding: '10px' }}>
                    <div className="flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-white">
                        <div>
                            ...
                            <button type="button"
                                    onClick={navigateToConcertUnMatch}
                                    class="px-4 py-2 text-sm font-medium text-gray-900 bg-transparent border border-gray-900 rounded-e-lg hover:bg-gray-900 hover:text-white focus:z-10 focus:ring-2 focus:ring-gray-500 focus:bg-gray-900 focus:text-white dark:border-white dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:bg-gray-700">
                                ยังไม่จับคู่
                            </button>
                        </div>
                        <p className="font-normal text-gray-500" style={{ marginLeft:"1170px" }}>วันที่จะถึง </p>
                        <svg className="w-[20px] h-[20px] text-gray-800 dark:text-white" onClick={navigateToPPageMΙΝ}
...
export default Management_Concert_MAX;
