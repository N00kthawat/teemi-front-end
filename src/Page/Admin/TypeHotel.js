...
function TypeHotel() {
    ...
    const [selectedTypeHotel, setSelectedTypeHotel] = useState({ ID_Type_Hotel: '', NameTH: '' });
    const API_URL = process.env.REACT_APP_API_URL;
    // Fetch room statuses from the API
    const fetchTypeHotel = async () => {
        try {
            const response = await fetch(`${API_URL}/admin-type-hotel?search=${search}`);
    ...
    const deleteTypeHotel = async (id) => {
        try {
            await fetch(`${API_URL}/admin-type-hotel/${id}`, {
                method: 'DELETE',
            });
            fetchTypeHotel(); // Refresh the list after deletion
        } catch (error) {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            if (editMode) {
                ...
                await fetch(`${API_URL}/admin-type-hotel/${selectedTypeHotel.ID_Type_Hotel}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ NameTH: selectedTypeHotel.NameTH }),
                });
            } else {
                // Add new room status
                await fetch(`${API_URL}/admin-type-hotel`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ NameTH: selectedTypeHotel.NameTH }),
                });
            ...
            setSelectedTypeHotel({ ID_Type_Hotel: '', NameTH: '' });
            setEditMode(false);
            fetchTypeHotel(); // Refresh the list after adding/editing
        } catch (error) {
    ...
    return (
        <>
            ...
            <form 
                ...
                onSubmit={(e) => { e.preventDefault(); fetchTypeHotel(); }}
            >   
                <label 
            ...
            <div style={{   width: '500px',  margin: '20px auto'  }} 
                ...
                <div  className="w-full h-64 overflow-y-auto bg-white border border-gray-300 rounded-lg dark:bg-gray-800 dark:border-gray-700"
                    ...
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                        {TypeHotel.map((type, index) => (
                            <tr 
                            key={type.ID_Type_Hotel} 
                            ...
                            <td className="px-6 py-4">{type.NameTH}</td>
                            ...
                            {index > 9 && (
                                    <button 
                                        ...
                                        onClick={(e) => {
                                            ...
                                            deleteTypeHotel(type.ID_Type_Hotel);
                                        }}
                                    >
                                        Delete
                                    </button>
            ...
            >   
                ...
                <div className="relative w-full">
                    <input 
                        ...
                        value={selectedTypeHotel.NameTH}
                        onChange={(e) => setSelectedTypeHotel({ ...selectedTypeHotel, NameTH: e.target.value })}
                        required 
                    />
                </div>
                <button 
