...
function TypeShow() {
    ...
    const [selectedTypeShow, setSelectedTypeShow] = useState({ ID_Type_Show: '', NameTS: '' });
    const API_URL = process.env.REACT_APP_API_URL;
    // Fetch room statuses from the API
    const fetchTypeShow = async () => {
        try {
            const response = await fetch(`${API_URL}/admin-type-show?search=${search}`);
    ...
    const deleteTypeShow = async (id) => {
        try {
            await fetch(`${API_URL}/admin-type-show/${id}`, {
                method: 'DELETE',
            });
            fetchTypeShow(); // Refresh the list after deletion
        } catch (error) {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            if (editMode) {
                ...
                await fetch(`${API_URL}/admin-type-show/${selectedTypeShow.ID_Type_Show}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ NameTS: selectedTypeShow.NameTS }),
                });
            } else {
                // Add new room status
                await fetch(`${API_URL}/admin-type-show`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ NameTS: selectedTypeShow.NameTS }),
                });
            ...
            setSelectedTypeShow({ ID_Type_Show: '', NameTS: '' });
            setEditMode(false);
            fetchTypeShow(); // Refresh the list after adding/editing
        } catch (error) {
    ...
    return (
        <>
            ...
            <form 
                ...
                onSubmit={(e) => { e.preventDefault(); fetchTypeShow(); }}
            >   
                <label 
            ...
            <div style={{   width: '500px',  margin: '20px auto'  }} 
                ...
                <div  className="w-full h-64 overflow-y-auto bg-white border border-gray-300 rounded-lg dark:bg-gray-800 dark:border-gray-700"
                    ...
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                        {TypeShow.map((type, index) => (
                            <tr 
                            key={type.ID_Type_Show} 
                            ...
                            <td className="px-6 py-4">{type.NameTS}</td>
                            ...
                            {index > 9 && (
                                    <button 
                                        ...
                                        onClick={(e) => {
                                            ...
                                            deleteTypeShow(type.ID_Type_Show);
                                        }}
                                    >
                                        Delete
                                    </button>
            ...
            >   
                ...
                <div className="relative w-full">
                    <input 
                        ...
                        value={selectedTypeShow.NameTS}
                        onChange={(e) => setSelectedTypeShow({ ...selectedTypeShow, NameTS: e.target.value })}
                        required 
                    />
                </div>
                <button 
