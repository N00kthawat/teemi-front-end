...
function TypeTicket() {
    ...
    const [selectedTypeTicket, setSelectedTypeTicket] = useState({ TID: '', NameT: '' });
    const API_URL = process.env.REACT_APP_API_URL;
    // Fetch room statuses from the API
    const fetchTypeTicket = async () => {
        try {
            const response = await fetch(`${API_URL}/admin-type-ticket?search=${search}`);
    ...
    const deleteTypeticket = async (id) => {
        try {
            await fetch(`${API_URL}/admin-type-ticket/${id}`, {
                method: 'DELETE',
            });
            fetchTypeTicket(); // Refresh the list after deletion
        } catch (error) {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            if (editMode) {
                ...
                await fetch(`${API_URL}/admin-type-ticket/${selectedTypeTicket.TID}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ NameT: selectedTypeTicket.NameT }),
                });
            } else {
                // Add new room status
                await fetch(`${API_URL}/admin-type-ticket`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ NameT: selectedTypeTicket.NameT }),
                });
            ...
            setSelectedTypeTicket({ TID: '', NameT: '' });
            setEditMode(false);
            fetchTypeTicket(); // Refresh the list after adding/editing
        } catch (error) {
    ...
    return (
        <>
            ...
            <form 
                ...
                onSubmit={(e) => { e.preventDefault(); fetchTypeTicket(); }}
            >   
                <label 
                ...
                <div  className="w-full h-64 overflow-y-auto bg-white border border-gray-300 rounded-lg dark:bg-gray-800 dark:border-gray-700"
                    ...
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        {TypeTicket.map((type, index) => (
                            ...
                            <td className="px-6 py-4">{type.NameT}</td>
            ...
            >   
                ...
                <div className="relative w-full">
                    <input 
                        ...
                        value={selectedTypeTicket.NameT}
                        onChange={(e) => setSelectedTypeTicket({ ...selectedTypeTicket, NameT: e.target.value })}
                        required 
                    />
                </div>
                <button 
