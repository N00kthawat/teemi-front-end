...
function MainHome() {
    ...
    const [selectedRoomStatus, setSelectedRoomStatus] = useState({ ID_Room_Status: '', NameRS: '' });
    const API_URL = process.env.REACT_APP_API_URL;
    // Fetch room statuses from the API
    const fetchRoomStatuses = async () => {
        try {
            const response = await fetch(`${API_URL}/admin-room-status?search=${search}`);
    ...
    const deleteRoomStatus = async (id) => {
        try {
            await fetch(`${API_URL}/admin-room-status/${id}`, {
                method: 'DELETE',
            });
            fetchRoomStatuses(); // Refresh the list after deletion
        } catch (error) {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            if (editMode) {
                ...
                await fetch(`${API_URL}/admin-room-status/${selectedRoomStatus.ID_Room_Status}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ NameRS: selectedRoomStatus.NameRS }),
                });
            } else {
                // Add new room status
                await fetch(`${API_URL}/admin-room-status`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ NameRS: selectedRoomStatus.NameRS }),
                });
            ...
            setSelectedRoomStatus({ ID_Room_Status: '', NameRS: '' });
            setEditMode(false);
            fetchRoomStatuses(); // Refresh the list after adding/editing
        } catch (error) {
    ...
    return (
        <>
            ...
            <form 
                ...
                onSubmit={(e) => { e.preventDefault(); fetchRoomStatuses(); }}
            >   
                <label 
            ...
            <div style={{   width: '500px',  margin: '20px auto'  }} 
                ...
                <div  className="w-full h-64 overflow-y-auto bg-white border border-gray-300 rounded-lg dark:bg-gray-800 dark:border-gray-700"
                    ...
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                        {roomStatuses.map((type, index) => (
                            <tr 
                            key={type.ID_Room_Status} 
                            ...
                            <td className="px-6 py-4">{type.NameRS}</td>
                            ...
                            {index > 1 && (
                                    <button 
                                        ...
                                        onClick={(e) => {
                                            ...
                                            deleteRoomStatus(type.ID_Room_Status);
                                        }}
                                    >
                                        Delete
                                    </button>
            ...
            >   
                ...
                <div className="relative w-full">
                    <input 
                        ...
                        value={selectedRoomStatus.NameRS}
                        onChange={(e) => setSelectedRoomStatus({ ...selectedRoomStatus, NameRS: e.target.value })}
                        required 
                    />
                </div>
                <button 
