...
function TypeRoom() {
    ...
    const [selectedTypeRoom, setSelectedTypeRoom] = useState({ ID_Type_Room: '', NameTR: '' });
    const API_URL = process.env.REACT_APP_API_URL;
    // Fetch room statuses from the API
    const fetchTypeRoom = async () => {
        try {
            const response = await fetch(`${API_URL}/admin-type-room?search=${search}`);
    ...
    const deleteTypeRoom = async (id) => {
        try {
            await fetch(`${API_URL}/admin-type-room/${id}`, {
                method: 'DELETE',
            });
            fetchTypeRoom(); // Refresh the list after deletion
        } catch (error) {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            if (editMode) {
                ...
                await fetch(`${API_URL}/admin-type-room/${selectedTypeRoom.ID_Type_Room}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ NameTR: selectedTypeRoom.NameTR }),
                });
            } else {
                // Add new room status
                await fetch(`${API_URL}/admin-type-room`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ NameTR: selectedTypeRoom.NameTR }),
                });
            ...
            setSelectedTypeRoom({ ID_Type_Room: '', NameTR: '' });
            setEditMode(false);
            fetchTypeRoom(); // Refresh the list after adding/editing
        } catch (error) {
    ...
    return (
        <>
            ...
            <form 
                ...
                onSubmit={(e) => { e.preventDefault(); fetchTypeRoom(); }}
            >   
                <label 
                ...
                <div  className="w-full h-64 overflow-y-auto bg-white border border-gray-300 rounded-lg dark:bg-gray-800 dark:border-gray-700"
                    ...
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        {TypeRoom.map((type, index) => (
                            ...
                            <td className="px-6 py-4">{type.NameTR}</td>
                            ...
                            {index > 11 && (
                                    <button 
                                        ...
                                        onClick={(e) => {
                                            ...
                                            deleteTypeRoom(type.ID_Type_Room);
                                        }}
                                    >
                                        Delete
                                    </button>
            ...
            >   
                ...
                <div className="relative w-full">
                    <input 
                        ...
                        value={selectedTypeRoom.NameTR}
                        onChange={(e) => setSelectedTypeRoom({ ...selectedTypeRoom, NameTR: e.target.value })}
                        required 
                    />
                </div>
                <button 
