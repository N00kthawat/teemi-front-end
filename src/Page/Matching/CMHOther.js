...
function CMHOther() {
    ...
    const navigateToCMHOtherPage = () => {
        navigate(`/cmh/${ID_user}/${ID_hotel}/${HDID}/${S_datetimeHD}/${E_datetimeHD}`);
    };
    ...
    const handleAddDeal = async () => {
        if (!endDate || !selectedDeal || !hotelDeals.length) {
            alert("กรุณาเลือกวันที่และดีล");
            return;
        }
    
        const numberOfRooms = hotelDeals.length > 0 ? Number(hotelDeals[0].Number_of_room) : 0;
    
        // แปลง NumOfRoom เป็นตัวเลข
        const requestedNumOfRooms = Number(numofroom.NumOfRoom);
    
        // ตรวจสอบจำนวนห้อง
        if (requestedNumOfRooms <= 0 || requestedNumOfRooms > numberOfRooms) {
            alert(`กรุณาใส่จำนวนห้องที่ถูกต้อง (1-${numberOfRooms}).`);
            return;
        }
        ...
        const data = {
            HDID: HDID,
            CDID: selectedDeal,
            Datetime_match: endDate.toISOString().split('T')[0],
            StatusD: 1,
            NumOfRoom: requestedNumOfRooms,
            Number_of_room: numberOfRooms
        };
    
        console.log('ข้อมูลที่กำลังส่ง:', data); // บันทึกข้อมูลเพื่อตรวจสอบ
    
        try {
            const response = await axios.post(`${API_URL}/add-deals-update-hdid-other`, data);
            if (response.status === 201) {
                alert('เพิ่มดีลสำเร็จ');
                setShowModal(false);
            } else {
                alert('ไม่สามารถเพิ่มดีลได้');
            }
        } catch (error) {
            if (error.response) {
                console.error('เกิดข้อผิดพลาดในการเพิ่มดีล:', error.response.data);
                alert(`ข้อผิดพลาด: ${error.response.data}`); // แสดงข้อความข้อผิดพลาดจากเซิร์ฟเวอร์
            } else {
                console.error('เกิดข้อผิดพลาดในการเพิ่มดีล:', error.message);
                alert('เกิดข้อผิดพลาดขณะเพิ่มดีล.');
            }
        }
    };
