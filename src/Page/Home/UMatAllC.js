...
function UMatAllC() {
    ...
    useEffect(() => {
        if (concertID) {
            console.log(`Fetching concerts CID: ${concertID}`); 
            const fetchConcerts = async () => {
                try {
                    const response = await axios.get(`${API_URL}/detailconcertsmt/${concertID}`);
    ...
    useEffect(() => {
        if (concertID) {
            console.log(`Fetching Chanel data for CID: ${concertID}`);
            const fetchChanelData = async () => {
                try {
                    const chanelResponse = await axios.get(`${API_URL}/chanelconcerts/${concertID}`);
    ...
    useEffect(() => {
        if (concertID) {
            console.log(`Fetching hotel data for CID: ${concertID}`);
            const fetchHotelData = async () => {
                try {
                    const hotelResponse = await axios.get(`${API_URL}/detail-con-in-hotel?CID=${concertID}`);
    ...
    return (
        <>
            ...
            {concerts.length > 0 ? (
                <ul style={{ marginTop: '-40px', maxWidth: '100%', width: '1200px' }}>
                    {concerts.map((concert, index) => (
                        <li key={concert.CID || index} className="mb-4 flex flex-col md:flex-row items-start">
                            <img className="w-full md:w-80 h-80 object-cover mb-4 md:mb-0 mr-0 md:mr-4" 
                            ...
                            <div className="flex flex-col justify-between p-4 leading-normal">
                                ...
                                <p><strong>วันที่ทำการแสดง:</strong> {concert.Pre_date}</p>
        ...
        <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700"
            ...
            {hotelData.length > 0 ? hotelData.map((hotel, index) => (
                <li key={hotel.HDID || index}>
                    <a href={`/umatallh/${hotel.ID_hotel}`} className="inline-flex font-medium items-center text-blue-600 hover:underline">
                        {hotel.NameH}
        ...
        <div style={{ marginTop: '10px', display: 'flex', justifyContent: 'center' }}>
            ...
            {concerts.length > 0 ? (
                <ul style={{ maxWidth: '100%', width: '1200px' }}>
                    {concerts.map((concert, index) => (
                        <li key={concert.CID || index} className="mb-4 flex flex-col md:flex-row items-start">
                            <div className="flex flex-col justify-between p-4 leading-normal">
                            ...
                            <img className="w-full md:w-80 h-80 object-cover mb-4 md:mb-0 ml-0 md:ml-4" 
                                src={concert.Poster} 
                                alt={concert.Name}
                                style={{ height:'100%' }}
                            />
        ...
        <div className="details-container" style={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
            <div className="details" style={{ maxWidth: '100%', width: '1200px' }}>
